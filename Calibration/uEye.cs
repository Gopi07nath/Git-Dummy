...
public class uEye
{
    ...
    // Added by sriram 10 april 2013 to fix TS1X-1107 this is the new enums declration for hotpixel correction from the camera .
    // ***********************************************
	// Hotpixel correction
	// ***********************************************
	...
	[DllImport(DRIVER_DLL_NAME ,EntryPoint="is_-")]			// is_InquireImageMem
    ...
    private static extern int is_SetIO(int phCam, int nIO);
    ...
    // Added by sriram 10 april 2013 to fix TS1X-1107 this is the new api declaration for hotpixel correction from the camera instead of the issetbadpixelcorrection.
    [DllImport(DRIVER_DLL_NAME, EntryPoint = "is_HotPixel")] // is_HotPixel
    unsafe private static extern int is_HotPixel(int hCam, uint nCommand, void* pParam, uint nSizeOfParam);
    ...
    public int SetTopoFlashMode()
    {
        ...
        ret = is_SetSubSampling(m_hCam, IS_SUBSAMPLING_2X_HORIZONTAL | IS_SUBSAMPLING_2X_VERTICAL);
        //IS_SUBSAMPLING_DISABLE);
        //
       
        ret = is_SetPixelClock(m_hCam, nClkFreq);
        ret = is_SetFrameRate(m_hCam, dFps, ref dFpsSet);
        ret = is_SetExposureTime(m_hCam, dExp, ref dExpSet);

        long pulDelay = 0;
        long pulDuration = 0;
        ret = is_GetGlobalFlashDelays(m_hCam, ref pulDelay, ref pulDuration);

        //Enable software trigger mode and set high-active flash mode.
        ret = is_SetExternalTrigger(m_hCam, IS_SET_TRIGGER_SOFTWARE);

        if (ret != IS_SUCCESS)
        {
            if (ret == IS_INVALID_EXPOSURE_TIME)
            {
                MessageBox.Show("Trigger mode not enabled; invalid exposure time!", "CalibrationTool", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                MessageBox.Show("Trigger mode not enabled; error code!", "CalibrationTool", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }

            //DEBUG_OUT(str, DEBUG_LEVEL_LOW);
            //AfxMessageBox(str, MB_ICONSTOP);
            return -1;
        }

        ret = is_SetFlashDelay(m_hCam, (int)pulDelay, (int)pulDuration);
        int IOMASK = is_SetIOMask(m_hCam, IS_GET_IO_MASK);
        ret = is_SetIOMask(m_hCam, 0x03);
        ret = is_SetIO(m_hCam, 0x00);
        
        ret = is_SetFlashStrobe(m_hCam, IS_SET_FLASH_OFF & ~(IS_SET_FLASH_IO_1 | IS_SET_FLASH_IO_2), 0);

        ret = is_SetHardwareGain(m_hCam, 50, IS_IGNORE_PARAMETER, IS_IGNORE_PARAMETER, IS_IGNORE_PARAMETER);
        ret = is_SetRopEffect(m_hCam, IS_SET_ROP_MIRROR_LEFTRIGHT | IS_SET_ROP_MIRROR_UPDOWN, 1, 0);

        return ret;
    }

    public int Set3NethraFlashMode(bool IsGlobalShutter,int flashDelay,int flashDuration)
    {
        int delay = flashDelay * 1000;
        int duration = flashDuration * 1000;

        double exp_min = 0, exp_max = 0, exp_iv = 0, exp_cur = 0;
        if (IsGlobalShutter)
        {
            GetExposureRange(ref exp_min, ref exp_max, ref exp_iv);
            exp_max -= 0.01;
            SetExposureTime(exp_max, ref exp_min);
            SetGlobalShutter(uEye.IS_SET_GLOBAL_SHUTTER_ON);
        }
        else
        {
            delay = 0;
        ...
        if (is_SetFlashDelay(m_hCam, delay, duration) == IS_SUCCESS)
            return is_SetFlashStrobe(m_hCam, IS_SET_FLASH_OFF & ~(IS_SET_FLASH_IO_1 | IS_SET_FLASH_IO_2), 0);
        return 1;
    }

    public int Set3NethraFlashModeOff()
    {
        if (m_hCam == 0)
            return IS_INVALID_CAMERA_HANDLE;

        return is_SetExternalTrigger(m_hCam, IS_SET_TRIGGER_OFF);
    }

    public int SetFlashContinuous(bool bVal, out bool bPrevVal)
    {
        int nMode = is_SetFlashStrobe(m_hCam, IS_GET_FLASHSTROBE_MODE, 0);
        bPrevVal = (nMode & IS_SET_FLASH_HIGH) == IS_SET_FLASH_HIGH;
        if (bVal)
            return is_SetFlashStrobe(m_hCam, IS_SET_FLASH_HIGH, 0);
        else
            return is_SetFlashStrobe(m_hCam, IS_SET_FLASH_LOW, 0);//IS_SET_FLASH_OFF
    }

    public int SetFlashEnabled()
    {   //Enable flash output pin (high active)
        return is_SetFlashStrobe(m_hCam, (IS_SET_FLASH_HI_ACTIVE), 0);
    }

    public int SetFlash2()
    {
        //is_SetFlashStrobe(m_hCam, (IS_SET_FLASH_HI_ACTIVE | IS_SET_FLASH_IO_2) & ~IS_SET_FLASH_IO_1, 0);
        //Enable first GPIO pin (flash is NAND output with flash output pin and GPIO)
        return is_SetIO(m_hCam, 0x00);     
    }

    public int SetFlash1()
    {
        //is_SetFlashStrobe(m_hCam, (IS_SET_FLASH_HI_ACTIVE | IS_SET_FLASH_IO_1) & ~IS_SET_FLASH_IO_2, 0);
        //Enable second GPIO pin (flash is NAND output with flash output pin and GPIO)
        return is_SetIO(m_hCam, 0x01); 
    }

    public int SetFlashOff()
    {
        //Set output IO bins to low active
        is_SetIO(m_hCam, 0x00); 
        //Disable flash output
        return is_SetFlashStrobe(m_hCam, IS_SET_FLASH_OFF, 0);
    }

    public bool SetIO(int nMask, int nVal, out int nPrevVal)
    {
        //TODO: folowing two lines are temporary code
        int IOMASK = is_SetIOMask(m_hCam, IS_GET_IO_MASK);
        is_SetIOMask(m_hCam, 0x03);

        nPrevVal = is_SetIO(m_hCam, uEye.IS_GET_IO); //returns curren GPIO status
        return is_SetIO(m_hCam, (nPrevVal & ~nMask) | (nVal & nMask)) == uEye.IS_SUCCESS;
    }

    public int SetGain(int val)
    {
        return is_SetHardwareGain(m_hCam, val, IS_IGNORE_PARAMETER, IS_IGNORE_PARAMETER, IS_IGNORE_PARAMETER);
    }

    public bool SetRGBGain(int nRed, int nGreen, int nBlue)
    {
        return is_SetHardwareGain(m_hCam, IS_IGNORE_PARAMETER, nRed, nGreen, nBlue) == IS_SUCCESS;
    }

    public bool GetRGBGain(ref int nRed, ref int nGreen, ref int nBlue)
    {
        nRed = is_SetHardwareGain(m_hCam, IS_GET_RED_GAIN, IS_IGNORE_PARAMETER, IS_IGNORE_PARAMETER, IS_IGNORE_PARAMETER);
        nGreen = is_SetHardwareGain(m_hCam, IS_GET_GREEN_GAIN, IS_IGNORE_PARAMETER, IS_IGNORE_PARAMETER, IS_IGNORE_PARAMETER);
        nBlue = is_SetHardwareGain(m_hCam, IS_GET_BLUE_GAIN, IS_IGNORE_PARAMETER, IS_IGNORE_PARAMETER, IS_IGNORE_PARAMETER);
        return true;
    }

    public int SetIllumination(bool val)
    {
        if (val)
        {
            return is_SetIO(m_hCam, 0x02);
        }
        else
        {
            return is_SetIO(m_hCam, 0x00);
    ...
    {
        int nTest = is_SetIO(m_hCam, IS_GET_IO);
        return ((nTest & 0x02) == 0x02);
    }
    ...
    public int ReadI2C(int nDeviceAddr, int nRegisterAddr, byte[] pbData, int nLen)
    {
        ...
        return is_ReadI2C(m_hCam, nDeviceAddr, nRegisterAddr, pbData, nLen);
    }
    ...
    public int WriteI2C(int nDeviceAddr, int nRegisterAddr, byte[] pbData, int nLen)
    {
        ...
        return is_WriteI2C(m_hCam, nDeviceAddr, nRegisterAddr, pbData, nLen);
    }



  /* Save image from image memory. TODO: create neater solution for intermediate bmp */
  /*
  public bool SaveImage(String fname)
  {
      string fname = "3Nethra_" + DateTime.Now.ToString("yyyyMMdd_HHmmss");
      byte[] b1 = System.Text.Encoding.UTF8.GetBytes(fname + ".bmp");
      if (is_SaveImage(m_hCam, b1) == uEye.IS_SUCCESS)
      {
    ...
    */
    ...
    public int SetImageAOI(ref int pXPos, ref int pYPos, ref int pWidth, ref int pHeight)
    {
        ...
        return is_SetAOI(m_hCam, IS_SET_IMAGE_AOI, ref pXPos, ref pYPos, ref pWidth, ref pHeight);
    }

	public bool SetSupertex(byte val) {
		byte[] retval = new byte[1];
		int result = is_ReadI2C(m_hCam, 0x46, val, retval, 1);
        //int result = is_WriteI2C(m_hCam, 0x46, val, retval, 1);
	...
	public bool SetIOExpanderConfig(byte val) {
		byte[] pbData = new byte[1];
		pbData[0] = val;
		int result = is_WriteI2C(m_hCam, 0xE0, 0x03, pbData, 1);
		return ( result == IS_SUCCESS );
	}

	public bool SetIOExpanderOutput(byte val, byte mask) {
		byte[] pbData = new byte[1];
		if ( is_ReadI2C(m_hCam, 0xE1, 0x01, pbData, 1) == IS_SUCCESS ) {
			pbData[0] = ( byte )( ( pbData[0] & ~mask ) | ( val & mask ) );
			int result = is_WriteI2C(m_hCam, 0xE0, 0x01, pbData, 1);
			return ( result == IS_SUCCESS );
		} else return false;
	}
    // Added by sriram 10 april 2013 to fix TS1X-1107 this is the new api definition for hotpixel correction from the camera instead of the issetbadpixelcorrection.
    unsafe public int HotPixel(uint nMode, void* pParam, uint SizeOfParam)
    {
        if (m_hCam == 0)
            return IS_INVALID_CAMERA_HANDLE;
